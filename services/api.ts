...
type BookingCreationData = {
    ...
    packageId: string; subPackageId: string; people: number; subAddOnIds: string[];
    notes: string; paymentMethod: string; paymentProof: File | null;
    referralCode: string;
    usePoints: boolean;
};

export const createBooking = async (formData: BookingCreationData): Promise<Booking> => {
    const batch = db.batch();
    const [allPackages, allAddOns, settings] = await Promise.all([getPackages(), getAddOns(), getSystemSettings()]);
    
    const client = await getOrCreateClient(formData.name, formData.email, formData.whatsapp);
    const clientRef = db.collection('clients').doc(client.email);
    ...
    if (selectedPackage.isGroupPackage && formData.people > 2) {
        extraPersonCharge = (formData.people - 2) * 15000;
    }
    ...
    const isNewClient = client.totalBookings === 0;

    if (isNewClient && formData.referralCode) {
        const validation = await validateReferralCode(formData.referralCode, formData.email);
        if (validation.valid) {
            ...
            batch.update(clientRef, { referredBy: referralCodeUsed });
        }
    } else if (!isNewClient) {
        const sortedTiers = [...settings.loyaltySettings.loyaltyTiers].sort((a,b) => b.bookingThreshold - a.bookingThreshold);
    ...
    if (formData.usePoints && client.loyaltyPoints > 0) {
        pointsValue = Math.min(
            ...
            client.loyaltyPoints * settings.loyaltySettings.rupiahPerPoint
        );
        ...
        batch.update(clientRef, { loyaltyPoints: firebase.firestore.FieldValue.increment(-pointsRedeemed) });
    }
    ...
    if (formData.paymentProof) {
        paymentProofFileName = formData.paymentProof.name;
        const proofRef = storage.ref(`payment_proofs/${Date.now()}-${formData.paymentProof.name}`);
        const snapshot = await proofRef.put(formData.paymentProof);
        paymentProofUrl = await snapshot.ref.getDownloadURL();
    }
    ...
    const newBookingData = {
        ...
        discountAmount, discountReason, referralCodeUsed, pointsRedeemed, pointsValue,
        extraPersonCharge
    };
    ...
    notificationService.notify({ recipient: UserRole.Admin, type: 'info', message: `Booking baru dari ${formData.name} menunggu konfirmasi.`, link: '/admin/schedule'});
    
    const doc = await docRef.get();
    return fromFirestore<Booking>(doc, ['bookingDate', 'createdAt']);
};
